...
# vetor_resultado: retorno da função com solução inicial
                   # Linha 1: íncide da cidade visitada
                   # Linha 2: Custo até a cidade a ser visitada
...
idx <- round(runif(1,1,3))  # Escolha aleatória entre as 3 cidades mais próximas
...
# Cálculo da solução da 2º cidade até n-1
...
for (i in 2:(n-1)){
  ...
  if(is.infinite(vetor_aux[2,idx]) == TRUE){ idx <- 1 } # Se solução for já visita, escolhe a mais próxima 
...
# Calcula distância mínima para a cidade restante (a cidade inicial 1)
{
vetor_aux <- matrix(0, nrow = 2, ncol = n)
vetor_aux[1,] <- seq(1, n, 1) 
